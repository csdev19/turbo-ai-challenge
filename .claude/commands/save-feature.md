...
allowed-tools: Read, Edit, Write, Glob, Grep, Skill, Bash(git log:_), Bash(git diff:_), Bash(git branch:_), Bash(ls:_), Bash(find:_), Bash(date:_)
...
## Step 8: Update CLAUDE.md with global context

After saving the feature docs, run the `claude-md-management:revise-claude-md` skill
to distill key conventions, gotchas, and patterns from the new feature into CLAUDE.md.

This ensures CLAUDE.md stays up-to-date with the best global summary without
requiring manual edits or duplicate pointers.
...
- CLAUDE.md updates from revise-claude-md
