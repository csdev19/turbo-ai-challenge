...
export default function LoginForm() {
  ...
  return (
    <div className={`flex flex-col items-center ${FONT_BODY}`}>
      <Image src="/login-image.png" alt="Welcome back" width={160} height={160} className="mb-6" priority />
      ...
      <form action={action} className="w-full space-y-4">
        ...
        <div className="relative">
          ...
          {state?.errors?.password && (
            <p className="mt-1 px-5 text-xs text-red-600">{state.errors.password[0]}</p>
          )}
        </div>
